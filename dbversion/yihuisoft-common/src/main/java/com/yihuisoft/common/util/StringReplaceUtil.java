...
public class StringReplaceUtil {
    ...
    public static String idCardReplaceWithStar(String idCard) {
        ...
        if (idCard==null || "".equals(idCard) || "null".equals(idCard)){
            return null;
        } else {
     ...
     * @return
     */
    public static String bankCardReplaceWithStar(Integer headN,String bankCard) {

        if (bankCard==null || "".equals(bankCard) || "null".equals(bankCard)) {
            return null;
        } else {
            if(bankCard.length() <= headN) {
                return bankCard;
            }
            return replaceAction(bankCard, "(?<=\\d{" + headN.toString() + "})\\d(?=\\d{4})");
        }
    }
